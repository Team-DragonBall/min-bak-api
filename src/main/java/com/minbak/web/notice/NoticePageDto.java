...
public class NoticePageDto<T> {
    ...
    private int endPage; // 페이지 네이게이션에서 끝 페이지
    private boolean hasPrev; // 이전 페이지가 있는지 여부 확인용
    private boolean hasNext; // 다음 페이지가 있는지 여부 확인용
    private List<T> objects; // 페이지에서 받아올 객체 리스트 데이터?

    // 그럼 여기까지가 아마 필드 설정?


    // 현재페이지, 페이지사이즈, 모든 아이템 수, 데이터리스트를 받아서 위 필드를 계산 후 주입
    ...
    // halfRange는 페이지 네비게이션에서 표시할 범위를 데이터에 맞게 동적으로 설정
        ...
        //시작페이지와 끝 페이지 계산 (1,2,3,4,5 페이지 네비게이션)
